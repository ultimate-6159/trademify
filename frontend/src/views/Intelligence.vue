<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-400 mt-1">
          20-Layer Omniscient Analysis System • {{ activeLayersCount }}/20
          Layers Active
        </p>
      </div>
    ...
    <div class="grid grid-cols-5 gap-4">
      <div
        class="bg-gradient-to-br from-purple-900/50 to-purple-700/30 rounded-lg p-4 text-center border border-purple-500/50"
      >
        <div class="text-purple-300 text-xs mb-1">🏛️ TITAN SCORE</div>
        <div class="text-3xl font-bold text-purple-400">{{ titanScore }}</div>
      </div>
      <div
        class="bg-gradient-to-br from-indigo-900/50 to-indigo-700/30 rounded-lg p-4 text-center border border-indigo-500/50"
      >
        <div class="text-indigo-300 text-xs mb-1">⚡ ULTRA 10x</div>
        <div class="text-3xl font-bold text-indigo-400">{{ ultraScore }}</div>
      </div>
      <div
        class="bg-gradient-to-br from-yellow-900/50 to-yellow-700/30 rounded-lg p-4 text-center border border-yellow-500/50"
      >
        <div class="text-yellow-300 text-xs mb-1">👑 SUPREME 20x</div>
        <div class="text-3xl font-bold text-yellow-400">{{ supremeScore }}</div>
      </div>
      <div
        class="bg-gradient-to-br from-cyan-900/50 to-cyan-700/30 rounded-lg p-4 text-center border border-cyan-500/50"
      >
      ...
      <div
        class="bg-gradient-to-br from-pink-900/50 to-pink-700/30 rounded-lg p-4 text-center border border-pink-500/50"
      >
    ...
    <div class="space-y-6">
      ...
      <div>
        <h3
          class="text-lg font-semibold text-white mb-3 flex items-center gap-2"
        >
          <span class="text-yellow-400">⚡</span> Ultra Intelligence Layers
          (17-20)
          <span
            class="text-xs bg-gradient-to-r from-yellow-500 to-pink-500 text-white px-2 py-1 rounded-full"
          >
            10x → 100x SMARTER
          </span>
        </h3>
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div
          v-if="lastDecisions.transcendent"
          class="bg-gray-700/50 rounded-lg p-4"
        >
          <div class="flex items-center gap-2 mb-3">
            <span class="text-xl">🌌</span>
            <span class="text-cyan-400 font-bold"
              >Transcendent Intelligence (50x)</span
            >
          </div>
          <div class="grid grid-cols-2 gap-2 text-sm">
            <div class="text-gray-400">Quantum State:</div>
            <div class="text-white">
              {{ lastDecisions.transcendent.quantum_state || "N/A" }}
            </div>
            <div class="text-gray-400">Win Probability:</div>
            <div class="text-green-400">
              {{ formatPercent(lastDecisions.transcendent.win_probability) }}%
            </div>
            <div class="text-gray-400">Score:</div>
            <div class="text-cyan-400">
              {{
                lastDecisions.transcendent.transcendent_score?.toFixed(0) || 0
              }}/100
            </div>
            <div class="text-gray-400">Can Trade:</div>
            <div
              :class="
                lastDecisions.transcendent.can_trade
                  ? 'text-green-400'
                  : 'text-red-400'
              "
            >
        ...
        <!-- Omniscient Decision -->
        <div
          v-if="lastDecisions.omniscient"
          class="bg-gray-700/50 rounded-lg p-4"
        >
          <div class="flex items-center gap-2 mb-3">
            <span class="text-xl">🔮</span>
            <span class="text-pink-400 font-bold"
              >Omniscient Intelligence (100x)</span
            >
          </div>
          <div class="grid grid-cols-2 gap-2 text-sm">
            <div class="text-gray-400">Consciousness:</div>
            <div class="text-white">
              {{ lastDecisions.omniscient.consciousness_level || "N/A" }}
            </div>
            <div class="text-gray-400">Edge:</div>
            <div class="text-green-400">
              {{ lastDecisions.omniscient.edge?.toFixed(2) || 0 }}%
            </div>
            <div class="text-gray-400">Score:</div>
            <div class="text-pink-400">
              {{
                lastDecisions.omniscient.omniscient_score?.toFixed(0) || 0
              }}/100
            </div>
            <div class="text-gray-400">Physics:</div>
            <div class="text-blue-400">
              {{ lastDecisions.omniscient.physics_state || "N/A" }}
            </div>
          </div>
          <!-- Prophecies -->
          <div
            v-if="lastDecisions.omniscient.prophecies?.length"
            class="mt-3 border-t border-gray-600 pt-2"
          >
            <div class="text-gray-400 text-xs mb-1">🔮 Prophecies:</div>
            <div
              v-for="prophecy in lastDecisions.omniscient.prophecies.slice(
                0,
                2,
              )"
              :key="prophecy"
...
const foundationLayers = computed(() =>
  allLayers.value.filter((l) => l.id >= 1 && l.id <= 10),
);
const advancedLayers = computed(() =>
  allLayers.value.filter((l) => l.id >= 11 && l.id <= 16),
);
const ultraLayers = computed(() =>
  allLayers.value.filter((l) => l.id >= 17 && l.id <= 20),
);

const activeLayersCount = computed(
  () => allLayers.value.filter((l) => l.active).length,
);
...
const transcendentScore = computed(() => {
  ...
  if (decision?.transcendent_score)
    return decision.transcendent_score.toFixed(0) + "%";
...
const omniscientScore = computed(() => {
  ...
  if (decision?.omniscient_score)
    return decision.omniscient_score.toFixed(0) + "%";
...
const getUltraLayerClass = (id) => {
  switch (id) {
    case 17:
      return "bg-gradient-to-br from-indigo-900/50 to-indigo-700/30 border-indigo-500";
    case 18:
      return "bg-gradient-to-br from-yellow-900/50 to-yellow-700/30 border-yellow-500";
    case 19:
      return "bg-gradient-to-br from-cyan-900/50 to-cyan-700/30 border-cyan-500";
    case 20:
      return "bg-gradient-to-br from-pink-900/50 to-pink-700/30 border-pink-500";
    default:
...
const refreshAll = async () => {
  ...
  try {
    ...
    console.log(
      "Layers loaded:",
      allLayers.value.length,
      "Active:",
      activeLayersCount.value,
    );
  } catch (e) {
  ...
  allLayers.value = [
    {
      ...
      icon: "🎯",
      description: "FAISS Pattern Matching",
      features: ["FAISS Index", "Z-Score", "Correlation"],
      active: false,
    },
    {
      id: 2,
      ...
      icon: "📊",
      description: "RSI, MACD, BB, EMA",
      features: ["RSI", "MACD", "BB", "EMA"],
      active: false,
    },
    {
      id: 3,
      ...
      icon: "📈",
      description: "Volume Profile, OBV",
      features: ["Volume Ratio", "OBV", "Spike"],
      active: false,
    },
    {
      id: 4,
      ...
      icon: "🔄",
      description: "H1/H4/D1 Confluence",
      features: ["HTF Trend", "Confluence"],
      active: false,
    },
    {
      id: 5,
      name: "Market Regime",
      icon: "🌊",
      description: "Trend/Range Detection",
      features: ["Trend", "Range", "Volatile"],
      active: false,
    },
    {
      id: 6,
      name: "Risk Guardian",
      icon: "🛡️",
      description: "2% Risk, 5% Daily Loss",
      features: ["Position Size", "Daily Limit"],
      active: false,
    },
    {
      id: 7,
      name: "Pro Features",
      icon: "🏆",
      description: "Session, News, Trailing",
      features: ["Session", "News", "Trailing"],
      active: false,
    },
    {
      id: 8,
      name: "Smart Brain",
      icon: "🧠",
      description: "Trade Journal, Pattern Memory",
      features: ["Journal", "Memory", "Adaptive"],
      active: false,
    },
    {
      id: 9,
      ...
      icon: "🎓",
      description: "Kelly Criterion, S/R",
      features: ["Kelly", "Regime", "S/R"],
      active: false,
    },
    {
      id: 10,
      ...
      icon: "📚",
      description: "Online Learning, Cycle",
      features: ["Online", "Cycle", "Optimize"],
      active: false,
    },
    {
      id: 11,
      name: "Neural Brain",
      icon: "🧬",
      description: "Pattern DNA, State Machine",
      features: ["DNA", "State", "Anomaly"],
      active: false,
    },
    {
      id: 12,
      ...
      icon: "🔮",
      description: "Cross-Asset, Predictive",
      features: ["Cross-Asset", "Predictive"],
      active: false,
    },
    {
      id: 13,
      ...
      icon: "⚛️",
      description: "Microstructure, Fractal",
      features: ["Smart Money", "GARCH", "Hurst"],
      active: false,
    },
    {
      id: 14,
      name: "Alpha Engine",
      icon: "🏅",
      description: "Order Flow, Liquidity",
      features: ["Order Flow", "POC", "Divergence"],
      active: false,
    },
    {
      id: 15,
      name: "Omega Brain",
      icon: "Ω",
      description: "Institutional, Manipulation",
      features: ["Big Money", "Stop Hunt"],
      active: false,
    },
    {
      id: 16,
      name: "Titan Core",
      icon: "🏛️",
      description: "Meta-Intelligence",
      features: ["Consensus", "Ensemble", "Self-Improve"],
      active: false,
    },
    {
      id: 17,
      ...
      icon: "⚡",
      description: "10x Smarter - SMC",
      features: ["SMC", "Session", "Volatility", "Liquidity"],
      multiplier: "10x",
      active: false,
    },
    {
      ...
      icon: "👑",
      description: "20x Smarter - Hedge Fund",
      features: ["Order Flow", "Entropy", "Fractal", "Win Prob"],
      multiplier: "20x",
      active: false,
    },
    {
      ...
      icon: "🌌",
      description: "50x Smarter - Quantum",
      features: ["Quantum", "7D Analysis", "Black Swan", "Purity"],
      multiplier: "50x",
      active: false,
    },
    {
      ...
      icon: "🔮",
      description: "100x Smarter - All-Knowing",
      features: ["Physics", "Neural", "Chaos", "Game Theory", "Prophecy"],
      multiplier: "100x",
      active: false,
    },
  ];
