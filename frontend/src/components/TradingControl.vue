  ...
  <div class="bg-gray-800 rounded-lg p-6">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center gap-3">
        <span class="text-sm text-gray-400">
          {{ tradingStore.enabled ? 'Active' : 'Inactive' }}
        </span>
        <button
          @click="toggleTrading"
          :class="[
        ...
        >
          <span
            :class="[
              ...
              tradingStore.enabled ? 'translate-x-7' : 'translate-x-1'
            ]"
          />
        </button>
      </div>
    ...
    <div class="grid grid-cols-4 gap-4 mb-6">
      <div class="bg-gray-700 rounded-lg p-4">
        ...
        <div :class="statusClass">{{ tradingStore.status }}</div>
      </div>
      
      <div class="bg-gray-700 rounded-lg p-4">
        <div class="text-gray-400 text-sm mb-1">Open Positions</div>
        <div class="text-2xl font-bold text-white">{{ tradingStore.openPositions }}</div>
      </div>
      
      <div class="bg-gray-700 rounded-lg p-4">
        <div class="text-gray-400 text-sm mb-1">Today P&L</div>
        <div :class="pnlClass">
          {{ tradingStore.todayPnl >= 0 ? '+' : '' }}{{ tradingStore.todayPnl.toFixed(2) }}
        </div>
      </div>
      
      <div class="bg-gray-700 rounded-lg p-4">
        <div class="text-gray-400 text-sm mb-1">Win Rate</div>
        <div class="text-2xl font-bold text-blue-400">{{ tradingStore.winRate.toFixed(1) }}%</div>
      </div>
    </div>
    ...
    <div class="flex gap-3 mb-6">
      ...
      <button
        ...
        class="px-4 py-2 bg-red-600 hover:bg-red-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg transition-colors"
      >
        🚫 Close All
      </button>
    ...
    <div class="bg-gray-700 rounded-lg p-4 mb-6">
      <div class="flex justify-between mb-2">
        <span class="text-gray-400">Daily Risk Used</span>
        <span class="text-white">{{ tradingStore.riskUsed.toFixed(1) }}% / {{ tradingStore.maxDailyLoss }}%</span>
      </div>
    ...
    <div v-if="tradingStore.paperTrading" class="bg-yellow-900/30 border border-yellow-700 rounded-lg p-4 flex items-center gap-3">
      <span class="text-2xl">📝</span>
      <div>
        <div class="text-yellow-400 font-semibold">Paper Trading Mode</div>
        <div class="text-yellow-600 text-sm">ไม่มีการเทรดจริง - ใช้เงินจำลอง</div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="showSettings" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
        <div class="bg-gray-800 rounded-xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-center justify-between">
              ...
              <input type="checkbox" v-model="settings.paper_trading" class="w-5 h-5">
            </div>
            ...
            <!-- Min Confidence -->
            <div>
              <label class="text-white block mb-2">Min Signal Confidence (%)</label>
              <input
                type="number"
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2">
                <label v-for="signal in allSignals" :key="signal" class="flex items-center gap-2 text-gray-300">
                  <input
                    type="checkbox"
                    :value="signal"
