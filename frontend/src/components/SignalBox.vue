<template>
  <div
    class="card text-center p-3 sm:p-4"
    :class="[signalClass, shouldPulse ? pulseClass : '']"
  >
    <!-- Signal Icon -->
    <div class="mb-3 sm:mb-4">
      <div
        class="w-14 h-14 sm:w-20 sm:h-20 mx-auto rounded-full flex items-center justify-center text-2xl sm:text-4xl"
        :class="iconBgClass"
      >
        {{ signalIcon }}
      </div>
    ...
    <!-- Signal Text -->
    <h2 class="text-xl sm:text-3xl font-bold mb-1 sm:mb-2">{{ signalText }}</h2>

    <!-- Confidence -->
    <div
      class="text-3xl sm:text-5xl font-bold mb-2 sm:mb-4"
      :class="confidenceColor"
    >
      {{ confidence }}%
    </div>
    <p class="text-gray-400 text-sm sm:text-base mb-3 sm:mb-4">Confidence</p>

    <!-- Signal Duration Countdown -->
    <div
      v-if="hasDuration && !isExpired"
      class="mb-3 sm:mb-4 p-2 sm:p-4 rounded-lg"
      :class="durationBgClass"
    >
      <div class="flex items-center justify-center gap-2 mb-1 sm:mb-2">
        <span class="text-sm sm:text-lg">⏱️</span>
        <span
          class="text-lg sm:text-2xl font-mono font-bold"
          :class="countdownColor"
        >
          {{ formattedCountdown }}
        </span>
      </div>
      <div class="text-xs sm:text-sm" :class="durationTextClass">
        {{ durationStatusText }}
      </div>
      <!-- Progress Bar -->
      <div class="mt-2 w-full bg-gray-700 rounded-full h-1.5 sm:h-2">
        <div
          class="h-1.5 sm:h-2 rounded-full transition-all duration-1000"
      ...
      <div class="mt-2">
        <span
          class="px-2 py-0.5 sm:py-1 rounded text-xs font-semibold"
    ...
    <div
      v-else-if="hasDuration && isExpired"
      class="mb-3 sm:mb-4 p-2 sm:p-3 bg-red-900/30 border border-red-500/50 rounded-lg"
    >
      <div class="flex items-center justify-center gap-2 text-red-400 text-sm">
        <span>⚠️</span>
        <span class="font-semibold">Signal Expired!</span>
      </div>
...
const shouldPulse = computed(() => {
  return (
    props.signal && props.signal.signal !== "WAIT" && confidence.value >= 70
  );
});

const pulseClass = computed(() => {
  if (!props.signal) return "";
  return ["STRONG_BUY", "BUY"].includes(props.signal.signal)
    ? "pulse-buy"
    : "pulse-sell";
});
</script>
