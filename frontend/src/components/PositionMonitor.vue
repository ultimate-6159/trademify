<template>
  <div class="bg-gray-800 rounded-lg p-3 sm:p-6">
    ...
    <!-- Empty State -->
    <div v-if="positions.length === 0" class="text-center py-4 sm:py-8">
      <div class="text-3xl sm:text-4xl mb-2">📭</div>
      <div class="text-gray-400 text-sm">No open positions</div>
    </div>

    <!-- Positions List -->
    <div v-else class="space-y-2 sm:space-y-3 max-h-80 overflow-y-auto">
      <div
        v-for="position in positions"
        :key="position.id"
        class="bg-gray-700 rounded-lg p-3 sm:p-4"
      >
        <div class="flex items-center justify-between mb-2 sm:mb-3">
          <div class="flex items-center gap-2 sm:gap-3">
            <span :class="sideClass(position.side)" class="text-xs sm:text-sm">
              {{ position.side }}
            </span>
          ...
          <div :class="pnlClass(position.pnl)" class="text-sm sm:text-base">
            {{ position.pnl >= 0 ? "+" : "" }}{{ position.pnl.toFixed(2) }}
            <span class="text-xs hidden sm:inline"
              >({{ position.pnl_percent.toFixed(2) }}%)</span
            >
          </div>
        </div>

        <div
          class="grid grid-cols-4 gap-2 sm:gap-4 text-xs sm:text-sm mb-2 sm:mb-3"
        >
          <div>
        ...
        <div
          class="relative h-1.5 sm:h-2 bg-gray-600 rounded-full mb-2 sm:mb-3"
        >
          <div
            v-if="position.stop_loss && position.take_profit"
            class="absolute h-1.5 sm:h-2 bg-blue-500 rounded-full"
            :style="progressStyle(position)"
          />
          <div
            class="absolute w-1.5 h-1.5 sm:w-2 sm:h-2 bg-white rounded-full transform -translate-x-1/2"
        ...
        <div class="flex gap-2">
          <button
            ...
            class="flex-1 py-1.5 sm:py-2 bg-gray-600 hover:bg-gray-500 text-white rounded text-xs sm:text-sm"
          >
            ✏️ <span class="hidden sm:inline">Modify</span>
          </button>
          <button
            @click="closePosition(position.id)"
            class="flex-1 py-1.5 sm:py-2 bg-red-600 hover:bg-red-700 text-white rounded text-xs sm:text-sm"
          >
            ✕ Close
          </button>
        </div>
    ...
    <div v-if="positions.length > 0" class="mt-4 pt-4 border-t border-gray-700">
      <div class="flex justify-between text-sm">
        <span class="text-gray-400"
          >Total Positions: {{ positions.length }}</span
        >
